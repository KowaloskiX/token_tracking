...
class TedTenderExtractor:
                                                ...
                                                if temp_path.exists() and temp_path.stat().st_size > 0:
                                                    ...
                                                    for (file_content, filename, preview_chars, original_bytes, original_filename) in file_results:
                                                        if filename.endswith('.txt'):
                                                            filename = f"{lang_text.lower()}_file.txt"
    ...
    async def execute(self, inputs: Dict) -> Dict:
        ...
        async with async_playwright() as p:
            logging.info("Launching headless browser for TED scraping...")
            ...
            try:
                ...
                logging.info(
                    f"TED extraction complete. Extracted {len(tenders)} tenders from {current_page} pages."
                )
