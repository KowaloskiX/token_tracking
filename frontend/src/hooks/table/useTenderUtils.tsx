...
export const useTenderUtils = () => {
  ...
  const calculateDaysRemaining = useCallback((deadlineStr: string): number => {
  ...
  const extractHour = useCallback((dateStr: string) => {
    if (!dateStr || dateStr.includes('NaN')) return '-';
  ...
  const calculateProgressPercentage = useCallback((createdAt: string, deadlineStr: string) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
