...
import { Settings, Eye, EyeOff, Plus, Minus, ChevronDown } from 'lucide-react';
...
export const ColumnManagement: React.FC<ColumnManagementProps> = ({
  ...
  const handleColumnToggle = (columnId: string, checked: boolean) => {
    console.log('Toggling column:', columnId, 'to', checked);
    onColumnVisibilityChange(columnId, checked);
  };

  const handleCriteriaAction = (criteriaName: string) => {
    const existingColumn = criteriaColumns.find(col => col.criteria_name === criteriaName);
    
    if (existingColumn) {
      if (existingColumn.visible) {
        console.log('Removing criteria column:', criteriaName);
        onRemoveCriteriaColumn(criteriaName);
      } else {
        console.log('Making criteria column visible:', criteriaName);
        onColumnVisibilityChange(existingColumn.id, true);
      }
    } else {
      console.log('Adding new criteria column:', criteriaName);
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center gap-1">
                ...
                <ChevronDown className={`h-3 w-3 transition-transform ${showCriteria ? 'rotate-180' : ''}`} />
              </div>
            </DropdownMenuItem>
                ...
                {availableCriteria.map((criteriaName) => {
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      onSelect={(e) => {
                        e.preventDefault();
                        handleCriteriaAction(criteriaName);
                      }}
